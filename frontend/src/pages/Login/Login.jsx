...
const Login = () => {
  ...
  const { login, register, checkEmail, loginWithGoogle, user } = useAuth();
          ...
          <div className="login-form-container">
            <header className="login-header">
              <h1 className="gradient-text">
                {step === "email" ? "Get Started" : mode === "login" ? "Welcome Back" : "Create Account"}
              </h1>
              <p>
                {step === "email" 
                  ? "Enter your email to sign in or create an account" 
                  : mode === "login" ? "Enter your password to continue" : "Please provide your details below"}
              </p>
            </header>

            {step === "email" && (
              <div className="social-login">
                <button type="button" className="social-btn" onClick={handleSocialLogin}>
            ...
            <div className="divider">
              <span>{step === "email" ? "or continue with email" : email}</span>
            </div>

            <form className="login-form" onSubmit={step === "email" ? handleContinue : handleSubmit} noValidate>
              {step === "email" ? (
                ...
                <>
                  ...
                  {mode === "login" && (
                    <div className="form-options">
                      ...
                      <a href="#" className="forgot-link" onClick={(e) => { e.preventDefault(); toast.info("Password reset coming soon!"); }}>
                        Forgot password?
                      </a>
                    </div>
                  )}
              ...
              >
                {isLoading ? (
                  <>
                    ...
                    {step === "email" ? "Checking…" : mode === "login" ? "Signing in…" : "Creating account…"}
                  </>
                ) : (
                  step === "email" ? "Continue" : mode === "login" ? "Sign In" : "Complete Registration"
                )}
              </button>
